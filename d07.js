...
Set.prototype.reduce = function(fn, s) { return [...this].reduce(fn, s); }
Set.prototype.minus = function(s) { s.forEach(v => this.delete(v)); return this; }
Set.prototype.union = function(s) { s.forEach(v => this.add(v)); return this; }

const part1input = raw.split('\n')
  .filter(x => !/no other|shiny gold bags contain/.test(x))
  .map(line =>
    line.replace(/ bags?.$/,'').split(/ bags contain \d+ | bags?, \d /g)
  )
;

const containedByShallow = part1input.reduce((m, [ key, ...vals ]) => {
  vals.forEach(val => m.set(
    val,
    (m.get(val) || new Set()).add(key)
  ));
  return m;
}, new Map());

let memo = new Set(), next = containedByShallow.get('shiny gold');

do {
  memo.union(next);
  next = next.reduce(
    (s, color) =>
      (containedByShallow.get(color)||[]).reduce(
        (ss, newColor) => ss.add(newColor),
        s
      ),
    new Set()
  ).minus(memo); // not strictly necessary but can be nice to remove redundancy
} while (next.size > 0);

console.log('part1', memo.size);

const getChildrenCount = (shallow => key =>
  shallow.get(key).reduce((s, [qty, color]) =>
    s + qty * getChildrenCount(color),
    1
  )
)(raw.split('\n')
  .map(line =>
    line.replace(/ bags?.$/,'').split(/ bags contain | bags?\.?, /g)
  ).reduce(
    (m, [key, ...vals]) =>
      m.set(key, vals[0] === 'no other' ? [] : vals.map(
        val => {
          const [ _, qty, color ] = val.split(/(\d+) /);
...
console.log('part2', getChildrenCount('shiny gold') - 1);
